...
export const FakeLoanModal = ({
  isQuizOpen,
  resetPage,
}: FakeLoanModalProps) => {
  <div className="text-center justify-start text-monochrome-700 text-lg font-medium leading-snug">
    은행직원이 어플설치 후 대출 신청서를작성하라고 한다면 어떻게 해야할까?{" "}
  </div>;
  return (
    <QuizModal
      isOpen={isQuizOpen}
      modalTitle={
        <p className="text-center justify-start text-monochrome-700 text-lg font-medium leading-snug">
          은행직원이 어플설치 후 대출 신청서를 작성하라고 한다면 어떻게
          해야할까?
        </p>
      }
      answerDesc={
        <>
          ...
          <span className="text-base font-medium leading-normal text-center">
            이 앱은 공식은행 대출신청 어플처럼 보이지만, 피싱범이 만든{" "}
            <span className="text-primary-400">공식은행사칭앱</span>으로, 어플
            설치 후 권한을 허용하면 당신의 스마트폰 화면을 사기범이 직접 조작할
            수 있게 되며, 대출신청서에 입력한 개인정보를 이용해서 범죄에 악용할
            수 있어요.
          </span>

          <span className="text-center text-highlight-1 leading-normal">
            <span className="text-base font-medium">
              정상적인 은행이나 금융기관은 먼저 대출을 권유하지 않습니다.
              <br />{" "}
            </span>
      ...
      correctAnswer={1}
      navigateTo={"/simulation/loan-investment/explain-fraud"}
    />
  );
};
