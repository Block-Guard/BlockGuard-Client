...
const ProsecutorMessage = () => {
  ...
  const [selectedSecondStep, setSelectedSecondStep] = useState(0);

  const onClickFirstStep = (selected: number) => {
    if (selectedFirstStep === 0) {
    ...
    <div className="w-full flex flex-col items-center">
      ...
      <div className="w-full flex flex-col gap-[30px] px-[15px]">
        <div className="p-[10px] max-w-[270px] w-fit bg-[#e9e9eb] rounded-[20px] text-[15px] text-start">
          안녕하세요, 서울중앙지방검찰청 수사관 김경찰입니다. 000씨 맞으신가요?
          지금 본인 앞으로 고소장이 접수가 되신 게 있어서 몇 가지 확인차 연락을
          드렸습니다
        </div>
        {step >= 1 && (
          <div className="flex flex-col items-end gap-[5px]">
            <button
              ...
              style={{
                background: selectedFirstStep === 1 ? "#437EFC" : "#D6ECF8",
                color: selectedFirstStep === 1 ? "#fff" : "#79818A",
              }}
            ...
            <button
              className="p-[10px] max-w-[270px] w-fit rounded-[20px] text-[15px] text-start"
              style={{
                background: selectedFirstStep === 2 ? "#437EFC" : "#D6ECF8",
                color: selectedFirstStep === 2 ? "#fff" : "#79818A",
              }}
            ...
            <button
              className="p-[10px] max-w-[270px] w-fit rounded-[20px] text-[15px] text-start"
              style={{
                background: selectedFirstStep === 3 ? "#437EFC" : "#D6ECF8",
                color: selectedFirstStep === 3 ? "#fff" : "#79818A",
              }}
        ...
        <div className="flex flex-col gap-[10px]">
          {step >= 2 && (
            <div className="p-[10px] max-w-[270px] w-fit bg-[#e9e9eb] rounded-[20px] text-[15px] text-start">
              본인 명의의 통장이 실질적으로 사용이 되었기 때문에 연락을 드렸고,
              지금 본인 명의 통장으로 불법 자금이 대략 300만원 가량 이용이
              되었는데
            </div>
          )}
          {step >= 3 && (
            <div className="p-[10px] max-w-[270px] w-fit bg-[#e9e9eb] rounded-[20px] text-[15px] text-start">
              본인 명의의 통장이 실제 범죄 자금 유통에 사용되었기 때문에 저희가
              계좌를 일시적으로 동결하려는 조치를 검토하고 있습니다.
            </div>
          )}
          {step >= 4 && (
            <div className="flex flex-col gap-[10px]">
        ...
        {step >= 5 && (
          <div className="flex flex-col items-end gap-[5px]">
            <button
              className="p-[10px] max-w-[270px] w-fit rounded-[20px] text-[15px] text-start"
              style={{
                background: selectedSecondStep === 1 ? "#437EFC" : "#D6ECF8",
                color: selectedSecondStep === 1 ? "#fff" : "#79818A",
              }}
            ...
            <button
              className="p-[10px] max-w-[270px] w-fit rounded-[20px] text-[15px] text-start"
              style={{
                background: selectedSecondStep === 2 ? "#437EFC" : "#D6ECF8",
                color: selectedSecondStep === 2 ? "#fff" : "#79818A",
              }}
            ...
            <button
              className="p-[10px] max-w-[270px] w-fit rounded-[20px] text-[15px] text-start"
              style={{
                background: selectedSecondStep === 3 ? "#437EFC" : "#D6ECF8",
                color: selectedSecondStep === 3 ? "#fff" : "#79818A",
              }}
