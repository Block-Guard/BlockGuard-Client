...
const AnalysisResultPage = () => {
  ...
  const makeForm = () => {
    ...
    type SurveyKey = typeof initSurvey & {
    ...
    for (const [key, value] of formData.entries()) {
      ...
      if (key !== "imageFiles") {
        for (const [key2, value2] of Object.entries(
          ...
          console.log(
            `FormData 중 ${key}를 키값으로 갖는 데이터 : ${key2} = ${value2}`
          );
        }
      } else if (key === "imageFiles") {
  ...
  useEffect(() => {
    const process = async () => {
      ...
      try {
        ...
        const detailDegree = {
          ...riskState[themeIndex],
          degree: (response.score * 180) / 100,
        };
        setResultTheme(detailDegree);
        setIsLoading(false);
      } catch (error) {
        console.error(
          "사기 분석 결과 페이지에서 로드 오류 발생 오류페이지 로드",
          error
        );
        setIsError(true);
      } finally {
  ...
  return (
    <div className="flex flex-col justify-between w-full h-full overflow-y-scroll">
      <Header
        leftChild={
          <button onClick={handleBackClick}>
            <svg
              className={overrideHeader ? "text-black" : "text-white"}
              width="10"
              height="19"
              viewBox="0 0 10 19"
              fill="none"
        ...
        title={
          <h1
            ...
            style={{ color: `${overrideHeader ? "#000B25" : "#fff"}` }}
          >
            AI 분석 결과
          </h1>
        }
      ...
      <div
        className="h-37 flex justify-between items-end mt-[57px] pt-6 pl-6 pr-9 overflow-clip"
        style={{ backgroundColor: `${resultTheme.bgColor}` }}
      >
        <div className="h-full flex items-start">
          <img
            src={resultTheme.bubbleChat}
            alt="위험 경고문구"
            className="w-70"
          />
        </div>
        <div className="h-full flex items-end">
          <img
            src={resultTheme.character}
            alt="캐릭터"
            className="relative top-7.5 right-3 w-45"
      ...
      <div className="flex flex-col items-center py-7.5 px-6 z-10">
        <div
          ...
          style={{ color: resultTheme.bgColor }}
        >
          {resultTheme.text}
        </div>
        ...
        {resultTheme.state === "safe" ||
        !data ||
        data.estimatedFraudType === "판단할 수 없음" ? null : (
          <TypeFeature fraudType={data.estimatedFraudType} />
        )}
      ...
      <BottomCard
        setOpenReportCall={setOpenReportCall}
        setOpenGuardianCall={setOpenGuardianCall}
      />
      <ReportCallDrawer
        openReportCall={openReportCall}
        setOpenReportCall={setOpenReportCall}
      />
      <GuardianCallDrawer
        openGuardianCall={openGuardianCall}
        setOpenGuardianCall={setOpenGuardianCall}
      />
    </div>
  );
